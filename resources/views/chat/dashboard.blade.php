<x-app-layout>
    ...
    <div class="max-w-2xl mx-auto p-4 sm:p-6 lg:p-8">
        <form method="POST" action="{{ route('chat.store') }}">
            ...
            <x-primary-button class="mt-4">{{ __('Chat') }}</x-primary-button>
        </form>
                                ...
                                <x-dropdown>
                                    ...
                                    <x-slot name="content">
                                        <form method="POST" action="{{ route('chat.destroy', $chat) }}">
                                            @csrf
                                            @method('delete')
                                            <x-dropdown-link :href="route('chat.destroy', $chat)" onclick="event.preventDefault(); this.closest('form').submit();">
                                                {{ __('Delete') }}
                                            </x-dropdown-link>
                                        </form>
                                    </x-slot>
